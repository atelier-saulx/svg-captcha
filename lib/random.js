...
const mathExprPlus = function(left, right, mathMaxResult){
	let result = left + right
	if (result > mathMaxResult) {
		left = mathMaxResult - left;
		right = Math.round((mathMaxResult - left) * Math.random())
		result = left + right
	}
...
const mathExprMinus = function(left, right, mathMaxResult){
	let result = left - right
	if (result > mathMaxResult) {
		left = Math.round(mathMaxResult * Math.random())
		right = Math.round((mathMaxResult - left) * Math.random())
		result = left - right
	}
	const text = result.toString();
 ...
 */
exports.mathExpr = function (min, max, operator, mathMaxResult = 9) {
	min = min || 1;
	max = max || mathMaxResult;
	const left = randomInt(min, max);
	const right = randomInt(min, max);
	switch(operator){
		case '+':
			return mathExprPlus(left, right, mathMaxResult)
		case '-':
			return mathExprMinus(left, right, mathMaxResult)
		default:
			return (randomInt(1, 2) % 2) ? mathExprPlus(left, right, mathMaxResult) : mathExprMinus(left, right, mathMaxResult);
	}
};
